...
######################################
    ...
    # hyperparameters from tuning
    CUDA_VISIBLE_DEVICES=0 python3 run.py --encode=0 --batch-size=16 --dropout=0.05 --frame-freq=30 --log=lstm_att_tuned --learning-rate=0.059125 --hidden-size=32 --optimizer=SGD --epochs=500 --mode=train --weight-decay=1e-05 --patience=30 --model=attention_lstm
...
######################################
    ...
    CUDA_VISIBLE_DEVICES=0 python3 run.py --encode=0 --hidden-size=32  --mode=test --log=test_rnn_att --model=attention_lstm --checkpoint=/mnt/disks/disk1/log/lstm_att_tuned_2020_6_7_h5_m40_lr0.059125/checkpoints/best_val_loss.pth
fi
